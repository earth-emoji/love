...
class EventListing(ListAPIView):
    ...
    def get_queryset(self):
        ...
        query_list = Event.objects.filter(campaign=campaign)
        ...
        if visibility:
            query_list = query_list.filter(visibility=visibility)
        if event_day:
            query_list = query_list.filter(start_time__day=event_day)
